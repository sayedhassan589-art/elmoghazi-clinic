...
(async () => {
  ...
  try {
    ...
    // Test other sub-tabs
    const subTabs = ['المتابعات', 'الجلسات', 'الزيارات', 'الأطباء', 'المخزون', 'الحجز', 'الأدوية', 'التقارير', 'النسخ', 'الملاحظات', 'الإعدادات'];
    for (const tab of subTabs) {
      try {
        ...
        if (tabBtn) {
          ...
          const hasErr = text.includes('حدث خطأ');
          if (hasErr) {
            console.log(`ERROR in ${tab}:`, text.substring(0, 200));
          } else {
            console.log(`OK: ${tab}`);
          }
        }
      } catch (e) {
        console.log(`Tab ${tab} error:`, e.message.substring(0, 100));
      }
    }
