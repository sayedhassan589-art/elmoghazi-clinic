...
export async function POST(request: Request) {
  try {
    ...
    // Helper: safely createMany, filtering out records with missing required fields
    const safeCreateMany = async (model: string, records: any[]) => {
      if (!records?.length) return 0
      try {
        // For SQLite, createMany doesn't skip duplicates by default.
        // We use skipDuplicates to avoid errors on re-import.
        ...
        // Try one-by-one as fallback for records that fail
        let count = 0
        for (const record of records) {
          try {
            ...
            // Skip individual records that fail (e.g., duplicate IDs)
          }
        }
        return count
      }
    ...
    // This must respect foreign key constraints
    const deleteOrder = [
